...
class VerifyTest(unittest.TestCase):
    ...
    def test_verify_design_and_conceptual_model_1(self):
        ...
        design = ts.Design(
            ...
            groupings=[
                sex.nested_under(classroom),
                extrav.nested_under(classroom),
            ],  # random slopes
        )

        cm = ts.ConceptualModel(
            associative_relationships=[
                sex.associate(popularity),
                extrav.associate(popularity),
                texp.associate(popularity),
            ]
        )
