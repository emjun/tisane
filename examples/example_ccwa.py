...
motivation = ts.Ordinal('motivation', order=[1, 2, 3, 4, 5, 6]) # Motivation score on a 6-point scale
...
treatment.treats(group, num_assignments=1) # Each group receives one treatment condition 
...
design = ts.Design(
    dv=pounds_lost, 
    ivs=[treatment, motivation]
).assign_data(df) # Load data
