...
class StatisticalModelTest(unittest.TestCase):
    def test_initialize_fixed_only(self):
        ...
        sm = ts.StatisticalModel(dv=acc, fixed_ivs=[expl])
    ...
    def test_initialize_fixed_interaction_1(self):
        ...
        # The graph IR has all the edges we expect
        self.assertEqual(
            len(sm.graph.get_edges()), 7
        )  # 1 for each IV and 2 for interaction (associate)
        # Main effects
    ...
    def test_initialize_random_ivs_1(self):
        ...
        for v in variables:
            self.assertTrue(sm.graph.has_variable(v))
        self.assertEqual(
            len(sm.graph.get_variables()), 5
        )  # 1 dv + 2 fixed + 1 random slopes + 1 interaction (student is not in statistical model's graph)
        variables_in_graph = sm.graph.get_variables()
        student_id = None
        school_id = None
        ixn_var = None
        ...
        self.assertIsNone(
            student_id
        )  # Student variable in the graph is not in the graph
        self.assertIsNotNone(
            school_id
        )  # School variable in the graph as an identifier node
    ...
    def test_initialize_random_intercepts_1(self):
        ...
        random_intercepts = [
            ts.RandomIntercept(groups=school),
            ts.RandomSlope(iv=hw, groups=student),
        ]
        sm = ts.StatisticalModel(
            dv=math,
        ...
        # The graph IR has all the variables
        self.assertEqual(
            len(sm.graph.get_variables()), 7
        )  # 1 dv + 3 fixed + 2 groups (student, school) + 1 interaction
        for v in variables:
            self.assertTrue(sm.graph.has_variable(v))
        variables_in_graph = sm.graph.get_variables()
        student_id = None
        ...
        self.assertIsNotNone(
            student_id
        )  # Student variable in the graph as an identifier node
        self.assertIsNotNone(
            school_id
        )  # School variable in the graph as an identifier node
