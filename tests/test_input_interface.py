...
class InputInterfaceTest(unittest.TestCase):
    @patch(
        "tisane.smt.input_interface.InputInterface.ask_inclusion_prompt",
        return_value="y",
    )
    ...
    @patch(
        "tisane.smt.input_interface.InputInterface.ask_multiple_choice_prompt",
        return_value=0,
    )
    def test_resolve_unsat(self, input):
        ...
        self.assertEqual(
            InputInterface.resolve_unsat(facts=facts, unsat_core=unsat_core), ufact_1
        )

    @patch(
        "tisane.smt.input_interface.InputInterface.ask_family_prompt", return_value=3
    )
    def test_ask_family_for_ordinal_dv(self, input):
        ...
        self.assertEqual(
            InputInterface.ask_family(options=options, dv=dv), gamma_family
        )
