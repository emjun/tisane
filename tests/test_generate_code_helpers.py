...
from tisane.main import construct_statistical_model, infer_family_functions, infer_link_functions, infer_main_effects_with_explanations, infer_interaction_effects_with_explanations, infer_random_effects_with_explanations
from tisane.code_generator import generate_statsmodels_formula, generate_statsmodels_family, generate_statsmodels_link, generate_statsmodels_model, generate_pymer4_formula
import tisane as ts
import pandas as pd
...
class GenerateCodeHelpersTest(unittest.TestCase):
    ...
    def test_generate_statsmodels_formula_main_interaction_random_slope(self):
        ...
        dv = subject.numeric("Dependent_variable", number_of_instances=time) # within-subject
        ...
        code = generate_pymer4_formula(statistical_model=sm)
        reference_code = "'Dependent_variable ~ Condition + Time + Time*Condition + (0+Time|Unit)'"
        self.assertEqual(code, reference_code)    
    ...
    def test_generate_statsmodels_formula_main_uncorrelated_random_slope_intercept(self):    
        subject = ts.Unit("Subject", cardinality=12)
        word = ts.Unit("Word", cardinality=4)
        condition = subject.nominal("Word_type", cardinality=2, number_of_instances=2)
        reaction_time = subject.numeric("Time", number_of_instances=word)
